  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 rounded-xl border border-gray-200 bg-white p-5 sm:grid-cols-5">
      <select
        ...
        className="col-span-2 rounded-lg border border-gray-300 px-3 py-2 sm:col-span-2"
      >
        {residents.map((r) => (
      ...
      <button type="submit" disabled={submitting} className="col-span-2 rounded-lg bg-brand py-2 font-medium text-white hover:bg-brand-dark disabled:opacity-50 sm:col-span-5">
        {submitting ? "กำลังออกบิล..." : "ออกบิล"}
      </button>
    </form>
  );
}
