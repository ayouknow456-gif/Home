            ...
            {bills.map((b) => {
              ...
              return (
                <tr key={b.id} className="border-b border-gray-50">
                  ...
                  <td className="p-3">{user?.nameTH ?? "—"}</td>
                  <td className="p-3">{b.month}/{b.year + 543}</td>
                  <td className="p-3">{b.units}</td>
                  <td className="p-3">{b.amount.toLocaleString()}</td>
                  <td className="p-3">{b.status === "paid" ? "✅ ชำระแล้ว" : "⏳ ยังไม่ชำระ"}</td>
                </tr>
              );
            })}
          </tbody>
