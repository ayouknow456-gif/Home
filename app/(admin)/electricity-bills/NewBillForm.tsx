...
interface Resident { uid: string; label: string; }
...
export default function NewBillForm({ residents }: { residents: Resident[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!userId || units <= 0) { toast.error("กรุณากรอกข้อมูลให้ครบ"); return; }
    setSubmitting(true);
    try {
      const res = await fetch("/api/admin/bills", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId, month, year, units }) });
      if (!res.ok) throw new Error(await res.text());
      toast.success("ออกบิลเรียบร้อย"); setUnits(0); router.refresh();
    } catch { toast.error("เกิดข้อผิดพลาด"); } finally { setSubmitting(false); }
  }
  return <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 rounded-xl border border-gray-200 bg-white p-5 sm:grid-cols-5">
    <select value={userId} onChange={(e) => setUserId(e.target.value)} className="col-span-2 rounded-lg border border-gray-300 px-3 py-2 sm:col-span-2">{residents.map((r) => <option key={r.uid} value={r.uid}>{r.label}</option>)}</select>
