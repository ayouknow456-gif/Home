  ...
  return (
    <aside className="flex h-screen w-60 flex-col bg-brand font-sarabun text-white">
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                active ? "bg-accent text-brand-dark font-semibold" : "text-white/85 hover:bg-brand-light"
              )}
            >
              <span>{item.icon}</span>
      ...
      <form action="/api/auth/logout" method="post" className="px-3 pb-6">
        <button className="w-full rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-brand-light">
          🚪 ออกจากระบบ
        </button>
      </form>
    </aside>
